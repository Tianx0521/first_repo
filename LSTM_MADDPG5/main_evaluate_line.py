...
def plot_velocity_magnitude(time_steps, velocities_magnitude):
    ...
    for i in range(len(velocities_magnitude)):
        if i != len(velocities_magnitude) - 1:
...
if __name__ == '__main__':
    env = UAVEnv(num_uav=3)
    n_agents = env.num_uav
    n_actions = 2
    ...
    velocities_magnitude = [[] for _ in range(env.num_uav)]  # record magnitude of vel
    velocities_x = [[] for _ in range(env.num_uav)]  # record vel_x
    velocities_y = [[] for _ in range(env.num_uav)]  # record vel_y

    maddpg_agents = MADDPG(state_dim=state_dim-2, other_state_dim=other_state_dim, hidden_dim=hidden_dim,
                           action_dim=action_dim, other_action_dim=other_action_dim,
                           n_agents=n_agents, n_actions=n_actions,
                           fc1=128, fc2=128,
                           alpha=0.0001, beta=0.003, scenario='UAV_Round_up1',
                           chkpt_dir='./tmp/lstm_maddpg/')

    maddpg_agents.load_checkpoint()
    print('---- Evaluating ----')

    states = env.reset_line()


    def update(frame):
        global states, other_states, velocities_magnitude, velocities_x, velocities_y
        ...
        actions = maddpg_agents.choose_action(states, total_steps, evaluate=True)
        #obs_, _, dones = env.step(actions)
        states_, rewards, dones = env.step_line(actions)
        env.render_anime(frame)
        states = states_
        if any(dones):
            ani.event_source.stop()
            print("Round-up finished in", frame, "steps.")
            # smoothed_velocities_magnitude = [[] for _ in range(env.num_uav)]
            # smoothed_velocities_x = [[] for _ in range(env.num_uav)]
            # smoothed_velocities_y = [[] for _ in range(env.num_uav)]
            # for i in range(env.num_uav):
